resource "aws_api_gateway_method" "account_confirm_put" {
...
resource "aws_api_gateway_integration" "account_confirm_put" {
  rest_api_id             = local.apigw_id
  resource_id             = aws_api_gateway_resource.account_confirm.id
  http_method             = "PUT"
  ...
  depends_on = [
    aws_api_gateway_method.account_confirm_put
  ]
}

resource "aws_api_gateway_method_response" "account_confirm_put" {
...
resource "aws_api_gateway_integration_response" "account_confirm_put" {
  for_each = local.api_status_response
  ...
  depends_on = [
    aws_api_gateway_method.account_confirm_put,
    aws_api_gateway_integration.account_confirm_put,
    aws_api_gateway_method_response.account_confirm_put
  ]
}
