...
resource "aws_lambda_permission" "validations" {
  statement_id  = "AllowExecutionFromCognito"
  action        = "lambda:InvokeFunction"
  principal     = "cognito-idp.amazonaws.com"
  source_arn    = aws_cognito_user_pool.testigo.arn
  function_name = "backend-cognito-validations-${var.environment}"
  qualifier     = "running"
}

resource "aws_iam_policy" "validations" {
  name = "lambda-cognito-validations-${var.environment}"
...
resource "aws_iam_role_policy_attachment" "validations" {
  role       = "lambda-cognito-validations-${var.environment}"
  policy_arn = aws_iam_policy.policy.arn
}
